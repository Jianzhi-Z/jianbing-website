...
const db = {
  ...
  admins: [{
    id: 1,
    username: 'admin',
    password_hash: bcrypt.hashSync('jianbing2024', 10),
...
app.post('/admin/articles', requireAuth, upload.single('cover_image'), (req, res) => {
  ...
  const newArticle = {
    ...
    content,
    excerpt,
    cover_image: req.file ? `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}` : null,
    category,
    tags,
    status,
    view_count: 0,
...
app.put('/admin/articles/:id', requireAuth, upload.single('cover_image'), (req, res) => {
  ...
  if (req.file) {
    article.cover_image = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
  }
...
app.post('/admin/products', requireAuth, upload.array('images', 5), (req, res) => {
  ...
  const images = req.files ? req.files.map(f => `data:${f.mimetype};base64,${f.buffer.toString('base64')}`).join(',') : '';
...
app.put('/admin/products/:id', requireAuth, upload.array('images', 5), (req, res) => {
  ...
  if (req.files && req.files.length > 0) {
    product.images = req.files.map(f => `data:${f.mimetype};base64,${f.buffer.toString('base64')}`).join(',');
  }
...
app.put('/admin/profile', requireAuth, upload.single('avatar'), (req, res) => {
  ...
  db.profile.title = title;
  db.profile.bio = bio;
  db.profile.email = email;
  db.profile.phone = phone;
  db.profile.location = location;
  db.profile.github = github;
  db.profile.twitter = twitter;
  db.profile.linkedin = linkedin;
  db.profile.wechat = wechat;
  db.profile.skills = skills;
  db.profile.updated_at = new Date().toISOString();
  
  if (req.file) {
    db.profile.avatar = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
  }
